  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/5 px-2 py-0.5 text-[11px] font-semibold text-primary">
            {isPickup ? (
              <>
                <Store className="h-3 w-3" /> Retirada no local
              </>
            ) : (
              <>
      ...
      {order.codigo_entrega &&
        ...
        (() => {
          const expired = order.codigo_expira_em
            ? new Date(order.codigo_expira_em) < new Date()
            : false;
          return (
            <div
              className={`rounded-2xl border-2 p-5 text-center shadow-lg ${expired ? "border-destructive bg-destructive/5" : "border-primary bg-gradient-to-br from-primary/10 to-primary/5"}`}
            >
              <p
                className={`text-xs font-bold uppercase tracking-wider ${expired ? "text-destructive" : "text-primary"}`}
              >
                {expired
                  ? "Código expirado"
                  : isPickup
              ...
              <p
                className={`mt-2 text-5xl font-black tracking-[0.5em] ${expired ? "text-destructive/60 line-through" : "text-primary"}`}
              >
                {order.codigo_entrega}
              </p>
          ...
          {activeStages.map((s, i) => {
            ...
            return (
              <div key={s.key} className="flex items-center gap-3">
                <div
                  className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${isDeliveredStep ? "bg-success text-white" : done ? "bg-primary text-primary-foreground" : current ? "bg-primary/20 text-primary ring-2 ring-primary" : "bg-muted text-muted-foreground"}`}
                >
                  {done || isDeliveredStep ? <CheckCircle2 className="h-3.5 w-3.5" /> : i + 1}
                </div>
                <span
                  className={`text-sm ${isDeliveredStep ? "font-bold text-success" : current ? "font-bold text-foreground" : done ? "text-foreground" : "text-muted-foreground"}`}
                >
                  {s.label}
                </span>
              </div>
      ...
      {delivery && delivery.lat != null && delivery.lng != null && (
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <p className="text-xs text-muted-foreground">
            Última posição: {delivery.lat.toFixed(5)}, {delivery.lng.toFixed(5)}
          </p>
          <a
            target="_blank"
            rel="noreferrer"
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="mt-3 space-y-1 border-t border-border pt-2 text-sm">
          ...
          <div className="flex justify-between text-muted-foreground">
            ...
            <span>
              {isPickup ? "Grátis" : order.frete_cents === 0 ? "Grátis" : fmt(order.frete_cents)}
            </span>
          </div>
      ...
      {CANCELABLE.has(order.status) && (
        <Button variant="outline" className="w-full" onClick={cancelar}>
          Cancelar pedido
        </Button>
      )}
