      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Defina pacotes que os estabelecimentos podem contratar.
          </p>
        </div>
          ...
          {plans.map((p) => (
            <div key={p.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              ...
              <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
                <li>
                  • Prioridade: <b>{p.prioridade}</b>
                </li>
                <li>
                  • Máx. anúncios: <b>{p.max_anuncios}</b>
                </li>
      ...
      <section>
        ...
        <div className="space-y-2">
          ...
          {pendentes.map((s) => (
            <div
              key={s.id}
              className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-500/40 bg-amber-500/5 p-4"
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold">
                  {s.establishments?.nome ?? "—"} <span className="text-muted-foreground">·</span>{" "}
                  {s.ad_plans?.nome ?? "—"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {brl(s.preco_pago_cents)} · solicitado em{" "}
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  variant="outline"
                  className="text-rose-600"
                  onClick={() => decidir(s, "rejected")}
                >
                  <X className="mr-1 h-3.5 w-3.5" /> Recusar
                </Button>
                <Button size="sm" onClick={() => decidir(s, "active")}>
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          {outros.slice(0, 30).map((s, i) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold">
                  {s.establishments?.nome ?? "—"} · {s.ad_plans?.nome ?? "—"}
                </p>
                <p className="text-xs text-muted-foreground">
                  <Clock className="mr-1 inline h-3 w-3" />
                  {s.fim_em ? `até ${new Date(s.fim_em).toLocaleDateString("pt-BR")}` : "—"} ·{" "}
                  {brl(s.preco_pago_cents)}
                </p>
              </div>
              <Badge
                variant={s.status === "active" ? "default" : "secondary"}
                className={s.status === "active" ? "bg-emerald-500 text-white" : ""}
              >
                {s.status}
              </Badge>
            </div>
          ...
          {outros.length === 0 && (
            <div className="p-6 text-center text-sm text-muted-foreground">Sem histórico.</div>
          )}
        </div>
      </section>
          ...
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  type="number"
                  value={f.preco_cents}
                  onChange={(e) => setF({ ...f, preco_cents: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={f.duracao_dias}
                  onChange={(e) => setF({ ...f, duracao_dias: Number(e.target.value) })}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={f.max_anuncios}
                  onChange={(e) => setF({ ...f, max_anuncios: Number(e.target.value) })}
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="flex items-center justify-between rounded-lg border border-border p-3">
                ...
                <Switch
                  checked={f.destaque_home}
                  onCheckedChange={(v) => setF({ ...f, destaque_home: v })}
                />
              </label>
              <label className="flex items-center justify-between rounded-lg border border-border p-3">
                <span className="text-sm">Destaque Categoria</span>
                <Switch
                  checked={f.destaque_categoria}
                  onCheckedChange={(v) => setF({ ...f, destaque_categoria: v })}
                />
              </label>
              <label className="flex items-center justify-between rounded-lg border border-border p-3">
                <span className="text-sm">Destaque na Busca</span>
                <Switch
                  checked={f.destaque_busca}
                  onCheckedChange={(v) => setF({ ...f, destaque_busca: v })}
                />
              </label>
              <label className="flex items-center justify-between rounded-lg border border-border p-3">
