...
function PedidosPage() {
  ...
  async function repetir(o: Order) {
    ...
    if (validos.length === 0) {
      toast.error("Nenhum item disponível");
      return;
    }
      ...
      <div className="flex gap-2">
        {(["todos", "ativos", "concluidos"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`rounded-full border px-3 py-1 text-xs font-medium ${filter === f ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p
                    className={`font-bold ${o.status === "delivered" ? "text-success" : "text-primary"}`}
                  >
                    {fmt(o.total_cents)}
                  </p>
                  {o.status === "delivered" ? (
                    ...
                    <Badge className="mt-1 bg-primary/15 text-primary hover:bg-primary/20">
