...
export const Route = createFileRoute("/_authenticated/admin/financeiro")({ component: FinanceiroPage });
...
function toCSV(rows: Row[]) {
  const header = ["created_at","order_id","establishment_id","gross_cents","commission_cents","courier_payout_cents","merchant_payout_cents","platform_revenue_cents","status"];
  const body = rows.map((r) => header.map((k) => (r as Record<string, unknown>)[k] ?? "").join(","));
...
function FinanceiroPage() {
  ...
  function download() {
    ...
    a.href = url; a.download = `wifome-financeiro-${range}d.csv`; a.click();
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Faturamento bruto, comissões, repasses e take-rate.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={download}><Download className="mr-1 h-4 w-4"/>CSV</Button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="border-b border-border p-4 text-sm font-semibold">Últimos lançamentos ({filtered.length})</div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-border text-xs text-muted-foreground">
              <tr className="[&>th]:px-4 [&>th]:py-2 [&>th]:text-left">
                <th>Data</th><th>Pedido</th><th>Bruto</th><th>Comissão</th><th>Loja</th><th>Entregador</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {isLoading && <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">Carregando…</td></tr>}
              {!isLoading && filtered.length===0 && <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">Sem lançamentos no período.</td></tr>}
              {filtered.map((r) => (
                <tr key={r.id} className="border-b border-border/50 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-2"><span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-xs text-amber-600">{r.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
