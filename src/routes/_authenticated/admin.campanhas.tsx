...
function CampanhasPage() {
  ...
  async function create() {
    ...
    setOpen(false);
    setF({ nome: "", descricao: "", audience: "all" });
        ...
        {data.map((c) => (
          <div key={c.id} className="rounded-xl border border-border bg-card p-5">
            <div className="flex items-start justify-between">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs ${c.status === "active" ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"}`}
              >
                {c.status}
              </span>
            </div>
            {c.descricao && (
              <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{c.descricao}</p>
            )}
