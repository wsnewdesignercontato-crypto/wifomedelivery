...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: revs } = await supabase
        .from("reviews").select("rating_loja").eq("establishment_id", estab.id);
      ...
      setK({
        ...
        aguardando, emPreparo, faturamentoHoje: faturamento,
        ticketMedio: ticket, cancelados,
        avaliacaoMedia: media, estoqueBaixo: baixo ?? 0,
      });
    })();
  }, [estab?.id]);

  if (!estab || !k) return <div className="text-sm text-muted-foreground">Carregando dashboard…</div>;
  ...
  const cards = [
    ...
    { icon: DollarSign, label: "Faturamento hoje", value: fmt(k.faturamentoHoje), color: "text-emerald-500" },
    { icon: DollarSign, label: "Ticket médio", value: fmt(Math.round(k.ticketMedio)), color: "text-emerald-500" },
    ...
    { icon: AlertTriangle, label: "Estoque baixo", value: k.estoqueBaixo, color: "text-orange-500" },
  ];
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="shadow-card">
          ...
          <CardContent className="space-y-2 text-sm">
            <p><Badge variant={estab.is_open ? "default" : "secondary"}>{estab.is_open ? "Aberta" : "Fechada"}</Badge></p>
            <p className="text-muted-foreground">Taxa de entrega: <span className="font-semibold">{fmt(estab.taxa_entrega_cents)}</span></p>
            <p className="text-muted-foreground">Pedido mínimo: <span className="font-semibold">{fmt(estab.pedido_minimo_cents)}</span></p>
            <p className="text-muted-foreground">Tempo médio: <span className="font-semibold">{estab.tempo_medio_min ?? 30} min</span></p>
          </CardContent>
        </Card>
        <Card className="shadow-card">
          <CardHeader><CardTitle className="text-sm">Dicas rápidas</CardTitle></CardHeader>
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            <p>• Cadastre horários de funcionamento em <b>Horários</b>.</p>
