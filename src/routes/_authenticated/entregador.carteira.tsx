...
type Withdrawal = { id: string; valor_cents: number; metodo: string; status: string; created_at: string };
...
function Carteira() {
  ...
  const disponivel = ledger.filter((l) => l.status === "paid").reduce((s, l) => s + l.courier_payout_cents, 0)
    - withdrawals.filter((w) => w.status === "pago" || w.status === "aprovado").reduce((s, w) => s + w.valor_cents, 0);
  const pendente = ledger.filter((l) => l.status === "pending").reduce((s, l) => s + l.courier_payout_cents, 0);
  ...
  async function load() {
    ...
    const { data: l } = await supabase.from("platform_ledger")
      ...
      .eq("courier_id", courier.user_id).order("created_at", { ascending: false }).limit(100);
    setLedger((l ?? []) as Ledger[]);
    const { data: w } = await supabase.from("courier_withdrawals")
      .select("id,valor_cents,metodo,status,created_at")
      .eq("courier_id", courier.user_id).order("created_at", { ascending: false }).limit(50);
  ...
  async function solicitar() {
    ...
    const { error } = await supabase.from("courier_withdrawals").insert({
      courier_id: courier.user_id, valor_cents: cents, metodo: "pix",
      pix_key: courier.pix_key, liquido_cents: cents,
    });
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-primary bg-gradient-brand p-6 text-primary-foreground shadow-brand">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="mt-3 bg-white text-primary hover:bg-white/90" size="sm" disabled={disponivel < 500}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">Chave PIX: {courier?.pix_key ?? "cadastre no perfil"}</p>
                <Button className="w-full" onClick={solicitar}>Confirmar</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <section>
        ...
        {ledger.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-border bg-card p-6 text-center text-sm text-muted-foreground">Sem lançamentos.</p>
        ) : (
          <div className="space-y-2">
            {ledger.slice(0, 20).map((l) => (
              <div key={l.id} className="flex items-center justify-between rounded-2xl border border-border bg-card p-3 shadow-card">
                <div>
