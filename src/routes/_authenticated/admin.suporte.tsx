...
function SuportePage() {
  ...
  const { data: msgs = [] } = useQuery({
    queryKey: ["ticket-msgs", sel?.id],
    queryFn: () => fetchMsgs(sel!.id),
    enabled: !!sel,
  });
  ...
  async function send() {
    ...
    const { error } = await supabase
      .from("support_messages")
      .insert({ ticket_id: sel.id, sender_id: u.user.id, mensagem: text.trim() });
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Tickets abertos por usuários e chat em tempo real.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-border bg-card">
          ...
          <div className="max-h-[calc(100vh-260px)] overflow-y-auto">
            ...
            {!isLoading && tickets.length === 0 && (
              <p className="p-4 text-sm text-muted-foreground">Nenhum ticket.</p>
            )}
            {tickets.map((t) => (
              <button
                key={t.id}
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span
                    className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] ${t.status === "open" ? "bg-amber-500/10 text-amber-600" : "bg-muted text-muted-foreground"}`}
                  >
                    {t.status}
                  </span>
                </div>
          ...
          {!sel ? (
            ...
            <>
              <div className="flex items-center justify-between border-b border-border p-4">
                <div>
                  <h3 className="font-semibold">{sel.assunto}</h3>
                  <p className="text-xs text-muted-foreground">Prioridade: {sel.priority}</p>
                </div>
