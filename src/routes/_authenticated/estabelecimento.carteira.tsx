...
function Carteira() {
  ...
  const disponivel =
    ledger.filter((l) => l.status === "paid").reduce((s, l) => s + l.merchant_payout_cents, 0) -
    withdrawals
      .filter((w) => w.status === "pago" || w.status === "aprovado")
      .reduce((s, w) => s + w.valor_cents, 0);
  const pendente = ledger
    .filter((l) => l.status === "pending")
    .reduce((s, l) => s + l.merchant_payout_cents, 0);
  ...
  async function solicitar() {
    if (!estab) return;
    if (kyc !== "aprovado")
      return toast.error("Conta não validada. Envie os documentos para aprovação.");
    ...
    const { error } = await supabase.from("establishment_withdrawals").insert({
      ...
      banco_info: {
        banco: estab.banco_nome,
        agencia: estab.banco_agencia,
        conta: estab.banco_conta,
        tipo: estab.banco_tipo,
      },
      liquido_cents: cents,
    });
        ...
        <div className="rounded-2xl border border-amber-300 bg-amber-50 dark:bg-amber-950/30 p-4 text-sm flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-800 dark:text-amber-300 mt-0.5">
              Envie contrato social, CNPJ e documento do titular em{" "}
              <b>Configurações → Documentos</b> para liberar saques.
            </p>
          </div>
        </div>
      )}
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button
                ...
                size="sm"
                disabled={disponivel < 500 || kyc !== "aprovado"}
              >
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl bg-muted p-3 text-xs space-y-1">
                  <p>
                    <b>PIX:</b> {estab?.pix_key ?? "—"}
                  </p>
                  <p>
                    <b>Titular:</b> {estab?.banco_titular ?? "—"}
                  </p>
                  <p>
                    <b>Doc:</b> {estab?.banco_documento ?? estab?.cnpj ?? "—"}
                  </p>
                </div>
            ...
            {withdrawals.map((w) => (
              ...
              >
                <div>
                  ...
                  {w.motivo_recusa && (
                    <p className="text-xs text-red-500 mt-1">Motivo: {w.motivo_recusa}</p>
                  )}
                </div>
                <Badge
                  variant={
