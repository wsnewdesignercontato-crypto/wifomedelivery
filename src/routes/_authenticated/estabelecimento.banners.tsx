...
function BannersPage() {
  ...
  const reload = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("estab_banners")
      .select("id,titulo,subtitulo,imagem_url,ativo,ordem")
  ...
  async function toggle(b: Banner) {
    await supabase.from("estab_banners").update({ ativo: !b.ativo }).eq("id", b.id);
    reload();
  }
  async function remover(id: string) {
    if (!confirm("Excluir banner?")) return;
    await supabase.from("estab_banners").delete().eq("id", id);
    reload();
  }
      ...
      <div className="grid gap-3">
        {items.map((b) => (
          ...
          >
            ...
            <Button size="icon" variant="ghost" onClick={() => remover(b.id)}>
              <Trash2 className="h-4 w-4 text-destructive" />
            </Button>
          </div>
        ))}
        {items.length === 0 && (
          <p className="text-sm text-muted-foreground">Sem banners cadastrados.</p>
        )}
      </div>
    </div>
  );
        ...
        <div>
          ...
          {f.imagem_url ? (
            ...
            <button
              ...
              onClick={() => fileRef.current?.click()}
              disabled={uploading}
              className="mt-2 flex h-32 w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border text-sm text-muted-foreground hover:border-primary hover:text-primary"
            >
              {uploading ? (
