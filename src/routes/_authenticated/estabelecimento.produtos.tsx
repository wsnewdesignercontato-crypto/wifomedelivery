...
type Variant = { id?: string; nome: string; preco_cents: number; ativo: boolean; ordem: number; _new?: boolean; _deleted?: boolean };
...
function ProdutoForm({ cats, addonGroups, produto, onSaved }: { cats: Categoria[]; addonGroups: { id: string; nome: string }[]; produto?: Produto; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase.from("product_variants").select("id,nome,preco_cents,ativo,ordem").eq("product_id", produto.id).order("ordem")
      .then(({ data }) => {
    ...
    for (const tpl of addonTpl) {
      ...
      if (!gid) {
        ...
        const rows = tpl.itens.map((it, i) => ({
          addon_group_id: gid!, nome: it.nome, preco_extra_cents: it.preco_extra_cents, ordem: i,
        }));
  ...
  function aplicarTemplateVariacoes() {
    ...
    const novas = variantTpl.map((v, i) => ({
      nome: v.nome, preco_cents: v.preco_cents, ativo: true, ordem: base + i, _new: true,
    }));
  ...
  async function salvar() {
    ...
    let descRaw = (form.descricao ?? "").replace(/\s*\[\[extras:[^\]]+\]\]\s*$/, "").trim();
      ...
      for (const v of variants) {
        if (v._deleted && v.id) {
          ...
          await supabase.from("product_variants").insert({
            product_id: productId!, nome: v.nome.trim(), preco_cents: v.preco_cents, ativo: v.ativo, ordem: v.ordem,
          });
        } else if (v.id && !v._deleted) {
          await supabase.from("product_variants").update({
            nome: v.nome.trim(), preco_cents: v.preco_cents, ativo: v.ativo, ordem: v.ordem,
          }).eq("id", v.id);
        }
      }
    }
    ...
    <div className="grid gap-3 max-h-[75vh] overflow-y-auto pr-1">
      <div>
        ...
        <Select value={form.menu_category_id} onValueChange={(v) => setForm({ ...form, menu_category_id: v })}>
          <SelectTrigger><SelectValue placeholder="Escolha a categoria primeiro" /></SelectTrigger>
          <SelectContent>
            {cats.map((c) => (<SelectItem key={c.id} value={c.id}>{c.nome}</SelectItem>))}
          </SelectContent>
        </Select>
      ...
      <div><Label>Nome</Label><Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder={kind === "bebida" ? "Ex: Coca-Cola" : kind === "lanche" ? "Ex: X-Bacon Artesanal" : kind === "pizza" ? "Ex: Pizza Calabresa" : "Nome do produto"} /></div>
      <div><Label>Descrição</Label><Textarea value={form.descricao.replace(/\s*\[\[extras:[^\]]+\]\]\s*$/, "")} onChange={(e) => setForm({ ...form, descricao: e.target.value })} placeholder={kind === "lanche" ? "Ingredientes, molhos, ponto sugerido..." : kind === "bebida" ? "Sabor, marca, embalagem..." : "Descreva o produto"} /></div>

      {extraDefs.length > 0 && (
        <div className="rounded-xl border border-border bg-muted/30 p-3">
          <p className="mb-2 text-xs font-semibold text-muted-foreground">Detalhes específicos de {KIND_LABEL[kind]}</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label>Estoque</Label><Input value={form.estoque} onChange={(e) => setForm({ ...form, estoque: e.target.value })} placeholder="Ilimitado" /></div>
      </div>
      <div className="flex items-center gap-2">
      ...
      {variantTpl.length > 0 || variants.filter((v) => !v._deleted).length > 0 ? (
        <div className="rounded-xl border border-border bg-card p-3">
          <div className="mb-2 flex items-center justify-between">
            <div>
              <Label>Variações {kind === "bebida" ? "(tamanhos/volumes)" : kind === "pizza" ? "(tamanhos)" : ""}</Label>
              <p className="text-xs text-muted-foreground">
                {kind === "bebida" ? "Ex: Lata 350ml, Garrafa 600ml, 1L, 2L — cada um com seu preço" :
                 kind === "pizza" ? "Ex: P, M, G, Família" :
                 kind === "acai" ? "Ex: 300ml, 500ml, 700ml, 1L" :
                 "Diferentes opções do mesmo produto"}
              </p>
            </div>
            <div className="flex gap-2">
              {variantTpl.length > 0 && (
                <Button size="sm" variant="outline" type="button" onClick={aplicarTemplateVariacoes}>
          ...
          <div className="space-y-2">
            {variants.map((v, i) => v._deleted ? null : (
              <div key={i} className="flex items-center gap-2">
                <Input className="flex-1" placeholder="Nome (ex: 350ml)" value={v.nome} onChange={(e) => updVariant(i, { nome: e.target.value })} />
                <Input className="w-28" placeholder="Preço R$" value={(v.preco_cents / 100).toFixed(2)}
                  onChange={(e) => updVariant(i, { preco_cents: Math.round(parseFloat(e.target.value.replace(",", ".") || "0") * 100) })} />
                <Switch checked={v.ativo} onCheckedChange={(a) => updVariant(i, { ativo: a })} />
                <Button size="icon" variant="ghost" type="button" onClick={() => delVariant(i)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
              </div>
            ))}
            {variants.filter((v) => !v._deleted).length === 0 && (
              <p className="text-xs text-muted-foreground">Sem variações. Se o produto tem só 1 tamanho/preço, use o Preço acima.</p>
            )}
          </div>
        </div>
      ) : null}
      ...
      <div className="rounded-xl border border-border bg-card p-3">
        <div className="mb-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {kind === "lanche" ? "Bacon, ovo, queijo extra, ponto da carne..." :
               kind === "pizza" ? "Borda, massa, sabores extras..." :
               kind === "bebida" ? "Gelo, copo, canudo..." :
               "Marque os grupos que o cliente poderá escolher."}
            </p>
          </div>
          {addonTpl.length > 0 && (
            <Button size="sm" variant="outline" type="button" onClick={aplicarTemplateComplementos} disabled={applyingTpl}>
        ...
        {addonGroups.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border p-3 text-center text-xs text-muted-foreground">
            Nenhum grupo cadastrado. {addonTpl.length > 0 ? "Clique em \"Aplicar template\" acima para criar automaticamente." : "Vá em Complementos e crie o primeiro."}
          </div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {addonGroups.map((g) => {
