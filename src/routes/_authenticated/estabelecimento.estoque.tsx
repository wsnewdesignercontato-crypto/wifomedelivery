...
function EstoquePage() {
  ...
  const reload = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("products")
      .select("id,nome,estoque,disponivel")
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Ajuste os níveis de cada produto. Produtos ficam indisponíveis automaticamente ao chegar em
        zero.
      </p>
      ...
      <td className="p-3">
        <div className="flex gap-2">
          <Input
            ...
            placeholder="Novo"
          />
          <Button size="sm" onClick={() => onChange(parseInt(v || "0"))}>
            Salvar
          </Button>
        </div>
      </td>
