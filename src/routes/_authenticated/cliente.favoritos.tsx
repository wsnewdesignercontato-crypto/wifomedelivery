...
function FavoritosPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("favorites")
      .select(
        "id,establishment:establishments(id,nome,descricao,capa_url,taxa_entrega_cents,tempo_medio_min,avaliacao,is_open)",
      )
      .eq("user_id", user.id);
                  ...
                  <Link to="/cliente/estabelecimento/$id" params={{ id: e.id }} className="block">
                    ...
                    <div className="p-3">
                      ...
                      <div className="mt-1 flex flex-wrap gap-2 text-xs text-muted-foreground">
                        ...
                        <span>
                          {e.taxa_entrega_cents === 0 ? "Grátis" : fmt(e.taxa_entrega_cents)}
                        </span>
                      </div>
                    </div>
                  </Link>
